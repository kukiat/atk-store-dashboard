...
- ข้อมูลหายเมื่อ restart process (seed กลับเป็น 5 คนจาก `src/modules/users/seed.json`)
...
```
outside --checkin--> waiting --verify pass--> inside --checkout--> paying --payment pass--> outside
                        '--verify fail--> outside                        '--payment fail--> paying (retry)
```

ทั้ง 4 transition ยิงผ่าน endpoint เดียว: `POST /users/:id/status` body `{ action, payload? }` (ดูหัวข้อ status ด้านล่าง)

POST /users (สร้างใหม่) = เดินเข้า + สแกนอัตโนมัติ → เริ่มที่ `inside` เลย
...
- `checkin` / `checkout` — ไม่มี payload
- `verify` / `payment` — ต้องมี `payload: { result: "pass" | "fail" }`
...
### checkout — `inside` → `paying`

ทิ้งทุกอย่างทันทีแล้วเดินไปที่ประตูทางออก **ยืนรอจ่ายเงินที่ fare-gate** (ยังไม่ออกจากร้าน — ต้องผ่าน payment ก่อน)
...
### payment — `paying` → `outside` (pass) / ค้างที่ประตู (fail)
...
curl -X POST http://localhost:3004/users/4/status \
  -H 'Content-Type: application/json' \
  -d '{"action":"payment","payload":{"result":"fail"}}'
```

## ฟัง event สด (SSE)

stream เดียวกับที่ dashboard ใช้ — event: `added` / `updated` / `removed` / `checkout` / `checkin` / `verify` / `payment` (มี `ping` ทุก ~8 วิเป็น keepalive) — event เหล่านี้ไม่ได้ยุบตาม HTTP route: service ยังยิงแยกต่อ transition เหมือนเดิม

...
- **ยกเว้น**: SSE (`GET /users/events`) ไม่ห่อซอง — มี framing ของตัวเอง (`event:` / `data:`); และ `GET /health` ก็ไม่ห่อ (liveness probe)
...
outside --enter--> waiting --verify pass--> inside --leave--> paying --pay pass--> outside
   '----verify------> '--verify fail--> outside                    '--pay fail--> paying (retry)
```

`verify` เรียกจาก `outside` ได้ตรง ๆ (รวบ step `enter` ให้เอง) — ยิงครั้งเดียวพาคนจากนอกร้านเข้าเลย (ดูหัวข้อ verify)
